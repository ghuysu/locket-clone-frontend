...
const SignInPage = ({ handleBackClick, setPage , setExpiryDay, setSignInKey, setUser}) => {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(content, { opacity: 0 }, { opacity: 1, duration: 1, delay: 1.5 });
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (rememberMe) {
          Cookies.set('rememberMeEmail', email, {secure: true, sameSite: 'Strict' });
          Cookies.set('rememberMePassword', password, {secure: true, sameSite: 'Strict' });
        } else {
      ...
      >
        ...
        <div className="mt-4 mr-4 flex items-center w-80 justify-end">
          <input
            ...
            className="mr-2"
          />
          <label htmlFor="rememberMe" className="text-gray text-xs medium">Remember me</label>
        </div>
        <div className='mt-4'>
          <SmallButton
...
const ForgotPasswordPage = ({handleBackClick}) => {
  ...
  return (
    <div>
      {isAuth ? 
      <ChangePassword verifiedEmail={verifiedEmail} handleBackClick={handleBackClick}/> : 
      <CheckEmailChangePassword setVerifiedEmail={setVerifiedEmail} setAuth={setAuth} handleBackClick={handleBackClick}/>}
    </div>
  );
}

export default function SignIn({handleBackClick, setExpiryDay, setSignInKey, setUser}) {
  ...
  return (
    <div>
      ...
      <SignInPage handleBackClick={handleBackClick} setPage={setPage} setUser={setUser} setSignInKey={setSignInKey} setExpiryDay={setExpiryDay}/>}
    </div>
  );
}

...
const ChangePassword = ({ verifiedEmail, handleBackClick }) => {
  ...
  const handleClick = async () => {
    ...
    try {
      const response = await fetch(
        'https://skn7vgp9-9876.asse.devtunnels.ms/access/password',
        {
          method: 'PATCH',
          headers: {
            'api-key': 'ABC-XYZ-WWW',
  ...
  return (
    <div className="bg-black flex flex-col w-full pt-10 border-t-4 border-yellow-500 rounded-lg">
      ...
      >
        <p className="bold text-2xl pb-7 text-gray">Let's change your password!</p>
        <div className="mt-4">
        ...
        {isSuccessfully && (
          <p className="text-green-500 text-base pt-4">Change password successfully</p>
        )}
      </div>
    </div>
  )

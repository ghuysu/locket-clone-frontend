...
function Email({oldEmail, setCode, setNewEmail, setPage, signInKey, userId}) {
    ...
    const handleClick = async () => {
        ...
        setError('');
        try {
            const response = await fetch('https://skn7vgp9-9876.asse.devtunnels.ms/account/email', {
                method: 'POST',
                headers: {
                    'api-key': 'ABC-XYZ-WWW',
    ...
    return (
        <div className="flex items-center justify-center bg-transparent" style={{ height: 'calc(100vh - 64px)' }}>
          <div ref={contentRef} className='bg-black w-[500px] h-[400px] p-8 rounded-3xl shadow-2xl border-t-4 border-yellow-500 flex flex-col justify-center items-center'>
            <p className="bold text-2xl pb-7 text-yellow-500">What is your new email?</p>
            ...
            <div className="flex flex-col justify-center space-x-4 pt-8">
              <p className="text-xs text-zinc-500 pb-2">Enter your new email along with your password <br></br> correctly to be processed</p>
              <Button
...
function Code({setPage, email, code, setUser, signInKey, userId}) {
    ...
    const handleClick = async () => {
        ...
        if(enteredCode !== code.toString()){
            ...
            setLoading(true)
            try {
                const response = await fetch('https://skn7vgp9-9876.asse.devtunnels.ms/account/confirm-email', {
                    method: 'PATCH',
                    headers: {
                        'api-key': 'ABC-XYZ-WWW',
                        'Content-Type': 'application/json',
    ...
    return (
         <div className="flex items-center justify-center bg-transparent" style={{ height: 'calc(100vh - 64px)' }}>
          <div ref={contentRef} className='bg-black w-[500px] h-[400px] p-8 rounded-3xl shadow-2xl border-t-4 border-yellow-500 flex flex-col justify-center items-center'>
            <p className="bold text-2xl pb-7 text-yellow-500">What's your received code?</p>
            <Input text={"Verify code"} handleChange={handleChange} name={"code"} value={enteredCode} />
            <p className="pt-2 text-xs text-zinc-600 pb-3">We sent an verify code to your new email, please check it!</p>
...
export default function EditEmail({user, setUser, signInKey}) {
    ...
    return (
        <div>
            {page==="code" ? 
            <Code setPage={setPage} email={newEmail} code={code} setUser={setUser} signInKey={signInKey} userId={user._id}/> : 
            <Email oldEmail={user.email} setCode={setCode} setNewEmail={setNewEmail} setPage={setPage} signInKey={signInKey} userId={user._id}/>}
        </div>
    );
}

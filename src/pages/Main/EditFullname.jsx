...
const EditFullname = ({user, signInKey, setUser}) => {
  ...
  useEffect(() => {
    ...
    if(firstname.length !== 0 && lastname.length !== 0 && newFullname !== oldFullname){
      setValid(true)
    } else {
      setValid(false)
    }
  ...
  const handleClick = async () => {
    ...
    try {
      const response = await fetch(
        'https://skn7vgp9-9876.asse.devtunnels.ms/account/name',
        {
          method: 'PATCH',
          headers: {
            'api-key': 'ABC-XYZ-WWW',
  ...
  return (
    <div className="flex items-center justify-center bg-transparent" style={{ height: 'calc(100vh - 64px)' }}>
      <div ref={contentRef} className='bg-black w-[500px] h-[400px] p-8 rounded-3xl shadow-2xl border-t-4 border-yellow-500 flex flex-col justify-center items-center'>
        <p className='text-3xl text-yellow-500 bold pb-20'>What is your new name?</p>
        <div>
          <Input
            text={"Firstname"}

...
const ImageFromCamera = ({ user, setUser, signInKey, setPage }) => {
      ...
      img.onload = async () => {
        ...
        canvas.toBlob(async (blob) => {
          ...
          const apiResponse = await fetch(
            `${import.meta.env.VITE_API_URL}/account/profile-image`,
            {
              method: "PATCH",
              headers: {
                "api-key": "ABC-XYZ-WWW",
  ...
  return (
    <div className="justify-center flex items-center bg-transparent h-[calc(100vh_-_64px)] max-h-[calc(100vh_-_64px)]">
      <div
        ref={contentRef}
        className="bg-black w-[500px] h-[700px] p-8 rounded-3xl shadow-2xl border-t-4 border-yellow-500 flex flex-col items-center"
      >
        <p className="text-2xl text-yellow-500 font-bold">
          Let's choose your new avatar!
        </p>
...
const ImageFromDevice = ({ user, setUser, signInKey, setPage }) => {
  ...
  return (
    <div className="flex items-center justify-center bg-transparent z-10 h-[calc(100vh_-_64px)] max-h-[calc(100vh_-_64px)]">
      <div
        ref={contentRef}
        className="bg-black lg:w-[900px] md:w-[700px] lg:h-[650px] md:h-[600px] p-5 rounded-3xl shadow-2xl border-t-4 border-yellow-500 flex flex-col items-center"
      >
        <p className="text-2xl text-yellow-500 font-bold">
          Let's choose your new avatar!
        </p>
